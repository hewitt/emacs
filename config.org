...
* Notes/preamble
+ Packages are installed separately via NixOS. Versions are fixed by the flake.
  
+ Emacs 30+ has 'use-package' (and some other things) built-in,
  notably 'treesitter'.

+ Emacs 30+ is required to use 'mode-line-format-right-align' in the
  modeline configuration below.
  
* Emacs 31, lexical binding cookie
[2026-06-26 Fri 17:10]  Temporary add?
...
Inhibit =Tramp= access to remote filesystem
...
typically do package provision via NixOS, but sometimes an interactive
interface can still be useful.
...
controlled by my NixOS configuration so these custom settings should
be treated as both transient and machine specific. This is usually
just a font choice for a given display.
...
that we immediately update any open buffers to show the on-file
current status.
...
** Straight :inactive:
Not used, but left in case of issues with a NixOS package.
...
** Modeline
This is a simplified home-brew modeline. Colours are defined to match
the current =ef-themes= choice. There is a hook to
=ef-themes-post-load-hook= to ensure that these are updated if the
=ef-theme= is updated.

Note from help for mode-line-format:

"The value may be nil, a string, a symbol or a list.

A value of nil means don’t display a mode line.

For any symbol other than t or nil, the symbol’s value is proocessed as
...
** Theme
Fonts: The default font is now set via =fontaine=.
...
** Frame borders/dividers    :inactive:

[2026-06-21 Sun 20:20] : not sure why this is here anymore. I'm leaving it
untangled and we will see if I notice any difference!
...
** Rainbow-delimiters
Colour brackets to make matching easier.
...
Trying v31.0.91 default completion with Prot's suggested tweaks, but still with =marginalia=
https://protesilaos.com/codelog/2026-07-29-emacs-default-minibuffer-completion-overview/

#+BEGIN_SRC emacs-lisp :tangle no
  ;; emacs v31+ improved completion sytem - applied to minibuffer
  (use-package minibuffer
    ...
    ;; history -- see below where this is set).
...
'consult-ripgrep' and 'consult-line', which resembles 'sniper'.
...
The =vertico=, =prescient=, =orderless= stack.
...
See: https://protesilaos.com/codelog/2026-04-30-emacs-decent-default-sacha-chua/
...
** Printer settings
This uses an external executable 'gtklp' to select the printer.
...
** Avy navigation
Jump to a position that is visible in the current buffer view.
...
Expand region is in maintenance mode only. Switched to expreg instead.
...
** Cut and paste :inactive:
I use Wayland (no X11), and this interacts with =wl-copy=.

In remote sessions you will need to wrap the ssh client in 'waypipe'
to get access to clipboard content.

March 2026: not sure that this is needed anymore. I'm trying to
disable it for a while so I can test cut/paste during
remote Zellij sessions.

#+BEGIN_SRC emacs-lisp :tangle no
  ;; - cut and paste in Wayland environment
  ;; - this puts selected text into the Wayland clipboard
  (setq select-enable-clipboard t)
  (defun rh-txt-cut-function (text &optional push)
    (with-temp-buffer
      (insert text)
      (call-process-region (point-min) (point-max) "wl-copy" ))
    )
  (setq interprogram-cut-function 'rh-txt-cut-function)
...
Shows git differences in the margin.
...
We use =hunspell= with the GB dictionary, enabling the global mode. The GB dictionary is set in the =Nix= flake config, by over-riding the =hunspell= package.

This is delbfdsftfritely wrong, should be highlighted
...
I use "m4" to update headers/footers/dates in LaTeX files for lecture
...
than trying to scribble a handwritten note via other methods.
...
* Email/mu4e
We need the =mu= package and also the executable =mbsync= (the package
that =mbsync= is in, is usually called =isync=). I run =davmail= as an
intermediary, with IMAP/SMTP.
...
Now, we can define the look/feel of how email is read and organised.
...
We need some quick elisp to start 'davmail' when 'mu4e' starts in order to
connect to O365 with MFA. We do a bit of a hack to stop the 'davmail' process
with a timer check every 15 minutes to see if 'mu4e' is still running or not.

[2026-07-31 Fri 18:27] : No longer used on a per-machine basis, instead I run a single instance on a specific machine.
