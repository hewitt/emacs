...
+ This relies on features only present in v31+
...
Inhibit =Tramp= access to remote filesystems.
...
[2026-08-04 Tue 17:59] I tried some of the built-in completion system of
v31.0.91 but still sticking with the =vertico= stack.
...
The =vertico=, =marginalia=, =orderless= stack.
...
[2026-08-06 Thu 10:25] not convinced yet, its set up to be interactive via C-z 
...
writing code and latex. This is handled in emacs by =envrc= instead.
...
Desktop notifications for AGE decryption - this is mostly the example from the
=org-caldav= README.
