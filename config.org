...
*On going issue* Dashboard is currently turned off because it causes
...
** Prot's themes 
I've switched away from doom-themes to the more systematic & consistent modus-* themes from Protesilaos Stavrou and more recently his ef-themes.

Fonts: in their infinite wisdom Dell decided that Linux editions of the XPS dev-edition laptop needed a hi-dpi panel (despite only being 13" diagonal). I run SwayWM and scale the panel display up by a factor of 2 for it to be readable, but when coupled with a more modest (1920x1200) external monitor the text on the lower resolution external display looks (admittedly only slightly) blurry. Eventually this annoyed me enough to swith to a bitmap font so I can relive the 80s but this time in a non-CRT focus. Font is now set via custom-settings.el.
