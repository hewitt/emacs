...
* Notes/preamble
+ Packages are installed via NixOS Home-Manager configuration. 
...
+ Terminal vs GUI.
  - Cut/Paste should still function in the terminal, but you may need
    shift+middle-mouse-button to paste from the clipboard.
...
intermediary, with IMAP/SMTP on localhost which seems to run well. The
=davmail= process is started as an asynchronous process under emacs as
needed when =mu4e= is started -- see =pod= section below.
